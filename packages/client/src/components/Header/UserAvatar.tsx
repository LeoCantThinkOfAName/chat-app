...
const UserAvatar = () => {
	...
	const handleClose = (e: React.MouseEvent<EventTarget>) => {
		if (avatarRef.current && avatarRef.current.contains(e.target as HTMLElement)) {
			return;
		}
	...
	return (
		<React.Fragment>
			<Button ref={avatarRef} onClick={handleClick} className={classes.button}>
				<StatusAvatar name={user.name[0]} thumbnail={user.thumbnail} status={user.status} />
			</Button>
			<Popper open={open} anchorEl={avatarRef.current} role={undefined} transition disablePortal>
					...
					>
						<Paper>
							<ClickAwayListener onClickAway={handleClose}>
								<MenuList autoFocusItem={true} id="user-status-menu" onKeyDown={handleListKeyDown}>
									{Object.keys(UserStatusColorScheme).map((key: string) => (
										<MenuItem key={key} onClick={(e) => handleSelect(e, key as UserStatus)}>
											<Box
												height={15}
												width={15}
												bgcolor={UserStatusColorScheme[key as UserStatus]}
												borderRadius="50%"
												mr={1}
											/>
											{t(`general.user.status.${key}`)}
										</MenuItem>
									))}
